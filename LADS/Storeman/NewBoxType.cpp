...
void __fastcall TfrmNewBoxType::cbFormsDropDown( TObject *Sender ) {
	...
	for( const LCDbBoxSize & bs : LCDbBoxSizes::records( ) ) {
		if( bs.isActive( ) && (expectedTubeType == 0 || bs.getTubeType( ) == expectedTubeType) ) {
...
void __fastcall TfrmNewBoxType::cbTypesDropDown( TObject *Sender ) {
	...
	for( const LPDbBoxType & bt : LPDbBoxTypes::records( ) ) {
		if( bt.isActive( )
		&& (currentFormation == NULL || bt.getSizeID() == currentFormation->getID())
		&& (bt.getProjectCID() == 0 || bt.getProjectCID() == LCDbProjects::getCurrentID())
		&& matchesExpected( bt.getAliquots( ) ) ) {
