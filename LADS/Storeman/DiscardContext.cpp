...
Context::isCreateJobStage( ) const
{
    ...
    switch (nextCrstatus)
    {
    case Cryovial::CONFIRMED: isCreateJobStage = true; break;
...
Context::isSelectJobStage( ) const
{
    ...
    switch (nextCrstatus)
    {
    case Cryovial::DESTROYED: isSelectJobStage = true; break;
    }
...
IntVec
Context::getAliquotIds( ) const
{
    return m_alinfo.getIds();
}

std::string
Context::getAliquotName( const int aliquotId ) const
{
    return m_alinfo.getName(aliquotId);
}

void
Context::configPersonFname( )
{
...
std::string
Context::getPersonFname( ) const
{
    return m_personFname;
}

IntVec
Context::getJobnos( ) const
{
	IntVec jobnos;
...
std::string
Context::calcJobPrompt( int jobno ) const
{
...
std::string
Context::calcJobDescription( int jobno ) const
{
    if (jobno == 0) jobno = m_jobno;
...
std::string
Context::calcJobStatus( int jobno ) const
{
    if (jobno == 0) jobno = m_jobno;
...
bool
Context::canAbort( int jobno ) const
{
    if (jobno == 0) jobno = m_jobno;
...
std::string
Context::getJobOwner( int jobno ) const
{
    if (jobno == 0) jobno = m_jobno;
...
std::string
Context::getJobDescription( int jobno ) const
{
    if (jobno == 0) jobno = m_jobno;
...
std::string
Context::getJobReason( int jobno ) const
{
    if (jobno == 0) jobno = m_jobno;
...
time_t
Context::getJobCreationUtime( int jobno ) const
{
    if (jobno == 0) jobno = m_jobno;
...
void
Context::setSearchType( const std::string & searchType )
{
...
const std::string &
Context::getSearchType( ) const
{
    return m_searchType;
}

void
Context::setSearchSource( const std::string & searchSource )
{
...
void
Context::configSearchTexts( const std::string & searchText )
{
    do
    {
        m_searchTexts.clear();
...
//        setText(cell, Util::asString(sampleno));
    }
