...
/** Constructs an object for managing the GUI, including setting up links to
  ...
  * When this constructor has finished, the WorklistEntriesView object is in a state
  ...
  */
WorklistEntriesView::WorklistEntriesView(TSnapshotFrame *m, DataManager* dm, LogManager* lm, const std::string & configFilename)
    :
    mainForm(m),
    dataManager(dm),
    logManager(lm),
...
/** Provides the width for this label that was calculated earlier.
  ...
  */
int WorklistEntriesView::textWidth(const std::string & label)
{
...
/** Calculates what the width of the given string would be, if displayed
  ...
  */
int WorklistEntriesView::findTextWidth(std::string s)
{
	TText *txt;
	txt = new TText(NULL);
	txt->AutoSize = true;
...
/** Notes the width of text labels for this entry.
  ...
  */
void WorklistEntriesView::setTextWidths(const DSampleTest & e, bool queued)
{
...
/** Sets up GUI components to correspond to the worklist entries for a particular
  ...
  */
void WorklistEntriesView::setUpWorklistEntryPanels(DSampleRun *s, TPanel *panel, bool q, int *w) {
...
/** Retrieves and displays the sample runs/QCs that are pending/completed
  ...
  */
void WorklistEntriesView::makeSamplesResultsDisplay() {
...
/** Retrieves and displays the samples that are queued (in the bottom half
  ...
  * cleared already.
  */
void WorklistEntriesView::makeWorklistQueueDisplay() {
...
void WorklistEntriesView::doGUIcalculations()
...
void WorklistEntriesView::setObserver( SnapshotFrameObserver *o )
{
    observer = o;
}

void WorklistEntriesView::setUpGUIcomponents()
{
	doGUIcalculations();
...
UnicodeString WorklistEntriesView::getComponentTagString(const TObject *comp)
...
Coordinates WorklistEntriesView::getComponentPosition(const TObject *comp)
{  // assumes that comp is a component (control) of some kind
...
TObject* WorklistEntriesView::getComponentParent(const TObject *comp)
{  // assumes that comp is a component (control) of some kind
...
/** Finds the ancestor of the given component that is immediately inside the
  ...
  */
TObject* WorklistEntriesView::findInnerPanelAncestor(TObject *comp)
