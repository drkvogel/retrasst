...
Positioning::Positioning(WorklistEntriesView *g)
	: entriesView(g)
{
	minTriOffset = entriesView->param("calloutTriangleOffset")
				   + entriesView->param("calloutTriangleWidth")/2;

	// these record the entriesView pane heights ratio
	recordResultsHeight = entriesView->param("initialResultsHeight");
	recordWorkHeight = entriesView->param("initialWorkHeight");
...
{   // decides whether the panel goes up top, left, right or down
	...
	float heightIfTB = p->innerSize.height + entriesView->param("calloutTriangleHeight");
	float widthIfLR = p->innerSize.width + entriesView->param("calloutTriangleHeight");
											  ...
											  int panelLocation)
{
	...
	if (panelLocation==TOP || panelLocation==BOTTOM) {
		...
		smartPos.size.height = p->innerSize.height
							   + entriesView->param("calloutTriangleHeight");
...
{	// will use a slightly more advanced positioning, depending on the
	...
	Block viewport;
	viewport.topLeft.x = entriesView->viewFrame->ResultsScrollBox->ViewportPosition.x;
	viewport.topLeft.y = entriesView->viewFrame->ResultsScrollBox->ViewportPosition.y;
	viewport.bottomRight.x = viewport.topLeft.x + entriesView->viewFrame->ResultsScrollBox->ClipRect.Width();
	viewport.bottomRight.y = viewport.topLeft.y + entriesView->viewFrame->ResultsScrollBox->ClipRect.Height();
...
void Positioning::positionInfoPanel(TInfoPanel *panel)
{
	...
	if (smartPos.triOffset>0) {  // triangle offset from lhs
		panel->CalloutOffset = smartPos.triOffset - entriesView->param("calloutTriangleWidth")/2;
	}
	else {
		panel->CalloutOffset = smartPos.triOffset + entriesView->param("calloutTriangleWidth")/2;
    }
	panel->CalloutWidth = entriesView->param("calloutTriangleWidth");
	panel->CalloutLength = entriesView->param("calloutTriangleHeight");
...
void Positioning::putLowerThan(TLabel *placed, TLabel *placing)
{
	...
	placing->Position->Y = placed->Position->Y
						   + entriesView->param("labelTextHeight");
}


/** Records the current heights of the two views (one for worklist entries
  * sent to the analyser, one for queued worklist entries), which are
  * controlled by a split pane. (Allows for keeping the same proportions
  * while resizing later.)
  *
  * @see adjustViewsHeights()
  */
void Positioning::recordViewsHeights()
{
		recordResultsHeight = entriesView->viewFrame->ResultsPanel->Height;
		recordWorkHeight = entriesView->viewFrame->WorkPanel->Height;
}

/** Adjusts the ratio of the height of the top view (for worklist entries
  * sent to the analyser) versus the height of the bottom view (for queued
  * worklist entries). This gets called when notification of a resize is
  * received.
  */
void Positioning::adjustViewsHeights()
{


	float oldTotalHeight = recordResultsHeight + recordWorkHeight;

	float newResultsHeight = entriesView->viewFrame->ResultsPanel->Height;
	float newWorkHeight =  entriesView->viewFrame->WorkPanel->Height;
	...
	if (totalHeight>0) {
		float improvedResultsHeight = recordResultsHeight * totalHeight
									  / oldTotalHeight;
		if (improvedResultsHeight < entriesView->param("minPanelHeight")) {
			 improvedResultsHeight = (float)entriesView->param("minPanelHeight");
		}

		entriesView->viewFrame->ResultsPanel->Height = improvedResultsHeight;
		entriesView->viewFrame->WorkPanel->Height = totalHeight - improvedResultsHeight;

		recordViewsHeights();
	}
	// Note: was having problems when the split pane was adjusted in
	// too extreme a ratio. Turns out that when you make the top pane
...
/* Sets the viewports of the two views (queued entries, analysed/ing entries)
 * to suitable positions. Called once the snapshot has been reloaded.
 * Specifically, the user can see the top of the view of queued worklist
 * entries, and the bottom of the view of worklist entries already sent
 * to the analyser.
 */
void Positioning::positionViews()
{
	// no need to set the bottom panel view (queued entries)
	// as the default is fine (at the top)

	// as for the top panel view,
	// this sets the viewport position to the bottom of the scrollbox
	entriesView->viewFrame->ResultsScrollBox->BeginUpdate();
	TPointF pos;
	pos.x = 0.0;
	pos.y = entriesView->viewFrame->ResultsInnerPanel->Height; // this position right at the end
												 // of the panel being viewed makes
												 // it go down to the bottom

	entriesView->viewFrame->ResultsScrollBox->ViewportPosition = pos;
	entriesView->viewFrame->ResultsScrollBox->EndUpdate();
	entriesView->viewFrame->ResultsScrollBox->Repaint();
